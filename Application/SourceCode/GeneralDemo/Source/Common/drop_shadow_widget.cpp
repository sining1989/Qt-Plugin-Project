...
namespace general_demo
{
	...
	/////////////////////////////////////////////////////////////////
	DropNoShadowWidget::DropNoShadowWidget(QWidget *parent)
		: QDialog(parent)
	{
		setWindowFlags(Qt::FramelessWindowHint | Qt::Dialog);
	...
	DropNoShadowWidget::~DropNoShadowWidget()
	{

	}

	void DropNoShadowWidget::paintEvent(QPaintEvent *)
	{
		QPainterPath path;
		path.setFillRule(Qt::WindingFill);
	...
	void DropNoShadowWidget::mousePressEvent(QMouseEvent *event)
	{
		//只能是鼠标左键移动和改变大小
		if (event->button() == Qt::LeftButton)
		{
	...
	void DropNoShadowWidget::mouseReleaseEvent(QMouseEvent *)
	{
		mouse_press = false;
	}

	void DropNoShadowWidget::mouseMoveEvent(QMouseEvent *event)
	{
		//移动窗口
		if (mouse_press)
		{
